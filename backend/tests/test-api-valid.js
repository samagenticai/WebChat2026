...
(async () => {
  ...
  try {
    ...
    if (d1.messages?.length > 0) {
      ...
      // attempt to reply to first message to ensure replyTo is supported
      const first = d1.messages[0];
      const replyRes = await fetch('http://localhost:5000/api/messages/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ identifier: first.sender?._id || first.recipient?._id, text: 'test reply', replyTo: first._id })
      });
      const rData = await replyRes.json();
      console.log('  ↳ reply attempt status', replyRes.status, 'replyTo field present?', !!rData.message?.replyTo);
    }
  } catch(e) {
